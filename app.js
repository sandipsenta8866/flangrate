...
// Make helper function global so other modules can use it
function inr(v, d) {
  try {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function computeOutputs(IN) {
    ...
    if (typeof calculator === "function") {
      // Pass helper values to the specific product's calculator function
  ...
  function populateProductTemplateDropdown() {
    const config = getCurrentConfig();
  ...
  function handlePDF() {
    ...
    html2canvas(printArea, { scale: 1.5, useCORS: true })
      .then((canvas) => {
        ...
        const topMargin = 15;
        pdf.addImage(imgData, "JPEG", 10, topMargin, imgWidth, imgHeight);
  ...
  function handleSaveQuotation() {
    ...
    const quotationData = {
      productName: currentProductKey,
      templateName: config.ROW_LABELS[currentRowKey],
      finalAmount: O.AA,
    ...
    firestoreManager.saveQuotation(quotationData);
  }
...
document.addEventListener("DOMContentLoaded", () => {
  // --- Global Constants ---
  // Note: 'n' and 'inr' are now in utils.js and are globally available
  const OUTPUT_DECIMALS = {
    I: 2,
    J: 2,
    K: 3,
